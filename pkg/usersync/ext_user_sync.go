...
func (es *ExtUserSyncAPI) registerRoutes() {
	...
	r.Post("/apiext/users/sync", reqGrafanaAdmin, bind(SyncExternalUserInfoCommand{}), wrap(es.SyncUser))
}
...
func (es *ExtUserSyncAPI) SyncUser(c *models.ReqContext) response.Response {
	...
	for _, role := range cmd.OrgRoles {
		if role.OrgID == -1 {
			if allOrgs == nil {
				...
				err := es.Bus.DispatchCtx(c.Req.Context(), query)
				if err != nil {
	...
	err := es.Bus.DispatchCtx(c.Req.Context(), upsert)
	if err != nil {
		return response.Error(500, err.Error(), err)
	}
